...
class CampaignList extends LoadView
{
	public function index($s = '', $id = '', $page = '1' )
	{
		...
		if ($s && $id) {
            $data['link'] = $s. '-c-'. $id;
            $data['page'] = $page;
            $data['campaignFind'] = $campaignModels->c_one(['slug' => $s, 'id' => $id]);
            if ($data['campaignFind']->category_id) {
				$categoryArray = explode(',', $data['campaignFind']->category_id);
			}
			if ($data['campaignFind']->brand_id) {
				$brandArray = explode(',', $data['campaignFind']->brand_id);
			}
			if ($data['campaignFind']->season) {
				$seasonArray = explode(',', $data['campaignFind']->season);
			}
            $data['categoryArray'] = $categoryArray;
            $data['brandArray'] = $brandArray;
            $data['seasonArray'] = $seasonArray;
			...
			$productCountWhere = $productCategoryModels->count_new_campaign(['is_active' => '1'], $id, '', $filterWhereAttrIn, $filterWhereAttrCombineIn, $filterWhereIn);
			...
			$data['productFind'] = $productCategoryModels->c_all_new_campaign(['is_active' => '1'], $id, ['whereStart' => $whereStart, 'item' => $item], $filterWhereAttrIn, $filterWhereAttrCombineIn, $filterWhereIn, $filterRank);
			...
			$filterCombinationArea = $productCategoryModels->filterCombinationAreaCampaign(['is_active' => '1'], $id, $filterWhereAttrIn, $filterWhereAttrCombineIn, $filterWhereIn);
			...
			$filterCategoryArea = $productCategoryModels->filterCategoryAreaCampaign(['is_active' => '1'], $id, $filterWhereAttrIn, $filterWhereAttrCombineIn, $filterWhereIn);
			if ($filterCategoryArea) {
				foreach ($filterCategoryArea as $keyParent => $row) {
					if ($row->cat_title) {
						$categoryTitleArray = explode(',', $row->cat_title);
						$categorySlugArray = explode(',', $row->cat_slug);
						if ($categoryTitleArray) {
							foreach ($categoryTitleArray as $key => $item) {
								$filterArrayCategory['title'][$keyParent] = $item;
								$filterArrayCategory['slug'][$keyParent] = $categorySlugArray[$key];
							}
						}
					}
				}
			...
			$filterOrderArea = $productCategoryModels->filterOrderAreaCampaign(['is_active' => '1'], $id, $filterWhereAttrIn, $filterWhereAttrCombineIn, $filterWhereIn);
			if ($filterOrderArea) {
			...
			$filterGenderArea = $productCategoryModels->filterGenderAreaCampaign(['is_active' => '1'], $id, $filterWhereAttrIn, $filterWhereAttrCombineIn, $filterWhereIn);
			if ($filterGenderArea) {
			...
			$filterBrandArea = $productCategoryModels->filterBrandAreaCampaign(['is_active' => '1'], $id, $filterWhereAttrIn, $filterWhereAttrCombineIn, $filterWhereIn);
			if ($filterBrandArea) {
			...
			$data['filterArrayCategoryTitle'] = array_unique($filterArrayCategory['title']);
			$data['filterArrayCategorySlug'] = array_unique($filterArrayCategory['slug']);
	...
	public function list($page = '1' )
	{
		...
		$productCategoryModels = new ProductCategoryModels($db);
		$attributeGroupModels = new AttributeGroupModels($db);
		$data['campaigns'] = $campaignModels->c_all_index(['is_active' => '1'], '');
		$data['productCount'] = count($data['campaigns']);
		$item = 12;
		if (!$page) {
			$page = 1;
		}
		...
		$data['totalPage'] = $totalPage;
		$data['campaigns'] = $campaignModels->c_all_index(['is_active' => '1'], ['whereStart' => $whereStart, 'item' => $item]);
		$this->viewLoad('campaigns', $data);
	}
}
