...
<?= view("admin/inculude/sidebar") ?>
    <main class="main-wrap">
        <?= view("admin/inculude/header") ?>
            <form id="mini_form" action="<?= base_url('productUpdate') ?>" onsubmit="return false">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="content-header">
                                <h2 class="content-title"><?= $productFind->title ?> Düzenleme</h2>
                                <div>
                        ...
                        <div class="col-lg-8">
                            <div class="card mb-4">
                                ...
                                <div class="card-body">
                                    <div class="mb-4">
                                        ...
                                        <input type="text" name="title" placeholder="Başlık" class="form-control" id="product_name" value="<?= $productFind->title ?>" />
                                    </div>
                                    <div class="mb-4">
                                        <label for="product_name_alt" class="form-label">Ürün Alt Başlığı</label>
                                        <input type="text" name="alt_title" placeholder="Alt Başlık" class="form-control" id="product_name_alt" value="<?= $productFind->alt_title ?>" />
                                    </div>
                                    <div class="mb-4">
                                        <label class="form-label">Ürün Açıklaması</label>
                                        <textarea placeholder="Açıklama" name="description" id="description" class="form-control" rows="4"><?= $productFind->description ?></textarea>
                                    </div>
                                    <div class="row">
                                        ...
                                        <div class="col-md-4">
                                            <div class="mb-4">  
                                                ...
                                                <input type="text" name="barcode_no" class="form-control" value="<?= $productFind->barcode_no ?>" />
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="mb-4">  
                                                ...
                                                <input type="text" name="stock" class="form-control" value="<?= $productCombination['0']->stock ?>" />
                                            </div>
                                        </div>

                                        <div class="col-lg-4 mb-4">
                                            <label class="form-label">Miat Süresi</label>
                                            <input class="form-control" type="date" name="expiration_date" value="<?= $productCombination['0']->expiration_date ?>"> 
                                        </div>
                                    </div>
                                    <div class="row">
                            ...
                            <div class="card mb-4">
                                ...
                                <div class="card-body">
                                    <div class="row draggable-zone" id="servicesPictureArea">
                                        <?php foreach ($productPicture as $row) : ?>
                                            <div class="col-md-3 col-6 col-6 mb-5" id="deleteItemArea-<?= $row->id ?>">
                                                <div class="image-input image-input-outline" data-kt-image-input="true" style="position: relative;">
                                                    ...
                                                    <div class="image-input-wrapper h-200px w-150px" >
                                                        <img src="<?= base_url() ?>/uploads/products/min/<?= $row->image ?>" alt="" />
                                                    </div>

                                                    <label style="position: absolute; right: -10px; top: -10px;border-radius: 50%;" class=" draggable-handle btn btn-icon btn-circle btn-active-color-primary w-25px h-25px bg-white shadow" data-kt-image-input-action="change" data-bs-toggle="tooltip" title="" data-bs-original-title="Sürükle ve Sırala">
                                                    ...
                                                    <span style="position: absolute; right: -10px; bottom: -5px;border-radius: 50%;" onclick="miniSingle('<?= $row->id ?>','<?= base_url('deleteProductImgEdit') ?>', 'deleteItem')" class="btn btn-icon btn-circle btn-active-color-primary w-25px h-25px bg-white shadow" data-kt-image-input-action="remove" data-bs-toggle="tooltip" title="" data-bs-original-title="Remove avatar">
                                    ...
                                    <div class="row gx-2">
                                        <div class="col-sm-12 mb-3">
                                            <div class="form-group mt-5">  
                                                    ...
                                                    <input type="hidden" name="parent_id" id="parent_id" value="<?= $productFind->category_id ?>">
                                            ...
                                            <div class="form-group mt-5">  
                                                ...
                                                <select name="brand_id" class="form-select form-select-solid">
                                                    ...
                                                    <?php foreach ($brandList as $row) : ?>
                                                        <option <?= $productFind->brand_id == $row->id ? 'selected' : '' ?> value="<?= $row->id ?>"><?= $row->title ?></option>
                                                    <?php endforeach ?>
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <?php foreach ($attributeGroupList as $row) : ?>
                                            <?php $attributListDetail =  $attributeModels->c_all(["a.is_active" => '2', 'ag.id' => $row->id]); ?>
                                            <div class="form-group mt-5">  
                                                ...
                                                <select name="productFeature[<?= $row->id ?>]" class="form-select form-select-solid" data-control="select2" >
                                                    <option value="0"><?= $row->title ?> Seçebilirsiniz.</option>
                                                    <?php foreach ($attributListDetail as $item) : ?>
                                                        <?php $attributFeature =  $productFeatureModels->c_one(["product_id" => $product_id, "attribute_group_id" => $row->id, "attribute_id" => $item->id ]); ?>
                                                        <option <?= $attributFeature ? 'selected' : '' ?> value="<?= $item->id ?>"><?= $item->title ?></option>
                                                    <?php endforeach ?>
                                                </select>
                                            </div>
                                        <?php endforeach ?>
                                        ...
                                        <div class="form-group mt-5">  
                                            ...
                                            <select name="is_active" class="form-select">
                                                <option value="1" <?= $productFind->is_active == '1' ? 'selected' : ''  ?>>Evet Ürün Diğer Kullanıclar tarafından kullanıla bilinsin.</option>
...
<script src="<?= base_url() ?>/public/admin/assets/js/jquery-ui.js"></script>
<script src="<?= base_url() ?>/public/admin/assets/js/jquery.ui.touch-punch.js"></script>
    ...
    window.addEventListener('load', function () {
        ...
        form.validate({
            ...
            submitHandler: function () {
                ...
                miniSubmit('' + action + '', 'product/list', '1');
            }
        });
    }, false);
...
<script>
    ...
    $(document).ready(function () {
        ...
        var $categoriesListJson =
            '[ { "id": "0", "text": "Ana Kategori", "parent" : "#", "state": { "opened": true, "visable":false }}, <?= rtrim($categoriesList, ',
            ') ?>]';
        var resetting = false;
        $('.menux').jstree({
            "core": {
       ...
       $('.draggable-zone').sortable({
            ...
            update: function (ui,event) {
                $('.draggable').each(function(e){
                    var nowlist = $(this).index();
                    $("#sortable li:eq("+ e +") span").html(nowlist+1);
                });
                var data =  $('.draggable-zone').sortable('serialize');
                $.ajax({
                    data: data,
                    type: 'POST',
                    url: '<?= base_url('pictureRankChange') ?>',
                    dataType: 'json',
